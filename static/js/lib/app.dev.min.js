...
initializeApp:function(){showLoginButton();app.preloadedTags.push("%photo","%video","%music","%voice","%book","%movie","%place","%weblink");for(var e=app.tag().getIconsInName(),f=0;f!=e.length;++f)app.preloadedTags.push("#"+e[f]);a(this);b();c();$("#year").effect("slide",{direction:"up"});app.getYears();d();animation.testCacheIcons();animation.testAllSubs()},addLoadDataWithFilter:function(a,b){if(""==b)animation.error(log.LOAD_DATA_FAIL+log.NO_CONTENT),animation.deny("#refresh-media");else{if(void 0==
...
b[c].time.start)/6E4);isNaN(f)||(app.displayedTime+=f)}$("#search-result").hide().fadeIn(500);$("#total-displayed").text(app.displayedNum);$("#total-char").text(app.displayedChars);$("#total-line").text(app.displayedLines);$("#total-time").text(app.displayedTime);if($("#list").get(0).scrollHeight!=$("#list").height()||++c==journal.total)break}else if(this.htmlEmpty(),++c==journal.total)break;++c>=journal.total&&($(".loadmore").remove(),$("#list").append('<li><p class="separator"><span>EOF</span></p></li>'));
...
$(document).ready(function(){app.app=$("div#app");app.contents=app.app.find(" > #contents");app.cList=app.app.find(" > #contents > #list");app.cDetail=app.app.find(" > #contents > #detail");app.itemView=_.template($("#list-view").html());app.detailView=_.template($("#detail-view").html());app.layout();app.initializeApp();archive.itemView=_.template($("#archive-view").html());archive.detailView=_.template($("#archive-detail-view").html());calendar.viewTemplate=_.template($("#calendar-view").html())});
...
window.calendar=function(){function a(a){for(var b=0;b!==c.length;++b)if(a>c[b][0])return c[b][1];return""}function b(b,c,e){b=new Date(b);var g=b.getMonth(),h=b.getDate();b=$("#month-"+g+" .day-"+h);c&&b.addClass("article");b.addClass(a(e)).css({article:c,bulb:e}).prepend("<div class='bubble'><p class='article-no'>"+c+"</p><p class='bulb-no'>"+e+"</p></div>");c||e?b.attr("href","javascript:;").click(function(){var a=app.monthArray[g]+" "+h;calendar.shrink(a);a="@"+(9>g?"0"+(g+1):g+1);a+=10>h?"0"+
h:h;a+=app.year%100;app.addLoadDataWithFilter(a)}):b.removeAttr("href").off("click")}var c=[[15,"bulb-3"],[7,"bulb-2"],[3,"bulb-1"],[1,"bulb-0"]];return{viewTemplate:void 0,initView:function(){var a=new Date,b=[];a.setMonth(0,0);for(var c=[],g=0;g<(a.getDay()+1)%7;++g)c.push(0);a.setFullYear(a.getFullYear()+1);for(var h=1;12>=h;++h){a.setMonth(h,0);for(g=1;g<=a.getDate();++g)c.push(g);for(;42>c.length;)c.push(0);b.push(c);c=[];for(g=0;g<(a.getDay()+1)%7;++g)c.push(0)}a=$(calendar.viewTemplate({days:b,
monthName:app.monthArray}));$("#data-calendar").html(a)},showContent:function(a){this.clear();a=journal.archive.data[app.year];for(var c=a.length,e=(new Date(app.year,0,1)).getTime(),g=0,h=0,k=0;k!==c;++k){var l=a[k],m=l.time.start||l.time.created;if(864E5<=m-e||0>m-e)b(e,g,h),h=g=0,e=new Date(m),e=(new Date(app.year,e.getMonth(),e.getDate())).getTime();l.contentType===app.contentType.BULB?++h:++g}},clear:function(){$(".calendar-table .day").removeClass("article bulb-0 bulb-1 bulb-2 bulb-3 bulb-4 bulb-5 bulb-6 bulb-7")},
shrink:function(a){a=a||"";$("#calendar-thumbnail").text(a);$("#data-calendar").addClass("minimized");$("#list").addClass("extended")},expand:function(){$("#data-calendar").removeClass("minimized");$("#list").removeClass("extended")}}}();
