...
window.calendar=function(){function a(a){for(var b=0;b!==c.length;++b)if(a>c[b][0])return c[b][1];return""}function b(b,c,e){b=new Date(b);b=$(".calendar-table .month-"+b.getMonth()+" .day-"+b.getDate());c&&b.addClass("article");b.addClass(a(e)).css({article:c,bulb:e}).children(".bubble").remove().prepend("<div class='bubble'><p class='article-no'>"+articleNum+"</p><p class='bulb-no'>"+bulbNum+"</p></div>");(c||e)&&b.attr("href","javascript:;").click(function(){})}var c=[[20,"bulb-7"],[15,"bulb-6"],
[10,"bulb-5"],[7,"bulb-4"],[5,"bulb-3"],[3,"bulb-2"],[2,"bulb-1"],[1,"bulb-0"]];return{viewTemplate:void 0,initView:function(){var a=new Date,b=[];a.setMonth(0,0);for(var c=[],g=0;g<(a.getDay()+1)%7;++g)c.push(0);a.setFullYear(a.getFullYear()+1);for(var h=1;12>=h;++h){a.setMonth(h,0);for(g=1;g<=a.getDate();++g)c.push(g);for(;42>c.length;)c.push(0);b.push(c);c=[];for(g=0;g<(a.getDay()+1)%7;++g)c.push(0)}a=$(calendar.viewTemplate({days:b,monthName:app.monthArray}));$("#data-calendar").html(a)},showContent:function(a){a=
journal.archive.data[app.year];for(var c=a.length,e=(new Date(app.year,0,1)).getTime(),g=0,h=0,k=0;k!==c;++k){var l=a[k],m=l.time.start||l.time.created;if(864E5<=m-e||0>m-e)b(e,g,h),g=!1,h=0,e=new Date(m),e=(new Date(app.year,e.getMonth(),e.getDate())).getTime();l.contentType===app.contentType.BULB?++h:g=!0}},clear:function(){$(".calendar-table .day").removeClass("article bulb-0 bulb-1 bulb-2 bulb-3 bulb-4 bulb-5 bulb-6 bulb-7")}}}();
