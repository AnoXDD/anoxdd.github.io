...
!a&&animation.warn(log.EDIT_PANE_TOO_MANY_RESULTS);for(var c=b.value,d=b=0,e=c.length;d!==e;++d){var f=c[d].size,g=c[d].id,h=c[d].name,k=h.substring(h.length-4).toLowerCase(),u=!1;if(".jpg"===k||".png"===k){for(var k=0,v=edit.photos;k!==v.length;++k)if(v[k].size==f){u=!0;break}u||(f={name:h,id:g,url:c[d]["@content.downloadUrl"],size:f,resource:!1,change:!1},++b,edit.photos.push(f))}}for(k=a?edit.photos.length-b:0;k!==edit.photos.length;++k)c=a?'<div class="queue">':edit.photos[k].resource?'<div class="resource">':
...
window.calendar=function(){function a(a){for(var b=0;b!==c.length;++b)if(a>c[b][0])return c[b][1];return""}function b(b,c,h){b=new Date(b);var k=b.getMonth(),l=b.getDate(),m=app.monthArray[k]+" "+l;b=$("#month-"+k+" .day-"+l);c&&b.addClass("article");b.addClass(a(h)).css({article:c,bulb:h}).prepend("<div class='bubble'>"+m+"<p class='article-no'>"+c+"</p><p class='bulb-no'>"+h+"</p></div>");d[k]=(d[k]||0)+(c||0);f[k]=(f[k]||0)+(h||0);c||h?b.attr("href","javascript:;").click(function(){calendar.shrink(m);
var a;a="@"+(9>k?"0"+(k+1):k+1);a+=10>l?"0"+l:l;a+=app.year%100;app.addLoadDataWithFilter(a)}):b.removeAttr("href").off("click")}var c=[[15,"bulb-3"],[7,"bulb-2"],[3,"bulb-1"],[1,"bulb-0"]],d={},f={};return{viewTemplate:void 0,initView:function(){var a=new Date,b=[];a.setMonth(0,0);for(var c=[],d=0;d<(a.getDay()+1)%7;++d)c.push(0);a.setFullYear(a.getFullYear()+1);for(var f=1;12>=f;++f){a.setMonth(f,0);for(d=1;d<=a.getDate();++d)c.push(d);for(;42>c.length;)c.push(0);b.push(c);c=[];for(d=0;d<(a.getDay()+
1)%7;++d)c.push(0)}a=$(calendar.viewTemplate({days:b,monthName:app.monthArray}));$("#data-calendar").html(a)},showContent:function(a){this.clear();a=journal.archive.data[app.year];for(var c=a.length,h=(new Date(app.year,0,1)).getTime(),k=0,l=0,m=0;m!==c;++m){var n=a[m],p=n.time.start||n.time.created;if(864E5<=p-h||0>p-h)b(h,k,l),l=k=0,h=new Date(p),h=(new Date(app.year,h.getMonth(),h.getDate())).getTime();n.contentType===app.contentType.BULB?++l:++k}for(a=0;12!=a;++a)c=d[a]||0,k=f[a]||0,$("#month-"+
a).prepend("<div class='bubble'><p class='article-no'>"+c+"</p><p class='bulb-no'>"+k+"</p></div>");a=new Date;if(app.year===a.getFullYear())for(a=a.getMonth()+1;12>a;++a)$("#month-"+a).addClass("hidden");a=new Date;app.year===a.getFullYear()&&$("#month-"+a.getMonth()+" .day-"+a.getDate()).addClass("today")},clear:function(){$(".calendar-table .day").removeClass("article bulb-0 bulb-1 bulb-2 bulb-3 bulb-4 bulb-5 bulb-6 bulb-7");d={};f={};$(".calendar-table").find(".bubble").remove();$(".month-cell").removeClass("hidden");
$(".today").removeClass("today")},shrink:function(a){a=a||"";$("#calendar-thumbnail").text(a);$("#data-calendar").addClass("minimized");$("#list").addClass("extended")},expand:function(){$("#data-calendar").removeClass("minimized");$("#list").removeClass("extended")}}}();
