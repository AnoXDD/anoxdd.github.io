...
4).toLowerCase();var h={id:h,name:l,title:l.substring(0,l.length-4),url:m,size:k,resource:!1,change:!0},n=!0;if(".mp4"===e){for(e=0;e!==edit.videos.length;++e)if(edit.videos[e].size===k){n=!1;break}n&&(++d,edit.videos.push(h),edit.addMedia(-1,{fileName:l,url:m,title:l.substring(0,l.length-4)}))}else if(".mp3"===e||".wav"===e){for(e=0;e!==edit.voices.length;++e)if(edit.voices[e].size===k){n=!1;break}n&&(++c,edit.voices.push(h),edit.addMedia(-3,{fileName:l,url:m,title:l.substring(0,l.length-4)}))}}edit.playableSetToggle();
...
1);for(var k=0;k!==f.length;++k)if(f[k].change){var p=f[k].name,q=f[k].title,w=f[k].id,r=c+((new Date).getTime()+k)+p.substring(p.length-4);f[k].success=!1;if(f[k].resource){for(var t=0;t!==g.length;++t)if(g[t].fileName===p){r=q+p.substring(p.length-4);break}q=e}else q=d;r={name:r,parentReference:{path:q}};w?p="https://api.onedrive.com/v1.0/drive/items/"+w+"?select=id,name,size,@content.downloadUrl&access_token="+l:(q=q===d?e:d,p="https://api.onedrive.com/v1.0"+q+"/"+encodeURI(p)+"?select=name,size,@content.downloadUrl&access_token="+
...
return a},f=function(a){animation.indent=0;animation.debug(log.CONTENTS_RELOADED);animation.testAllSubs();console.log("==================Force loaded==================");$("#list").empty();app.lastLoaded=0;app.lastQualified=-1;app.displayedChars=0;app.displayedLines=0;app.displayedNum=0;app.displayedTime=0;app.currentDisplayed=-1;app.command=a;$("#query").val(a);$("#total-displayed").text(app.displayedNum);$("#total-char").text(app.displayedChars);$("#total-line").text(app.displayedLines);$("#total-time").text(app.displayedTime);
...
app.list.prototype={load:function(a){var b=journal.archive.data[app.year],c=app.lastLoaded,d=app.lastQualified;app.lastLoaded>=journal.archive.data[app.year].length&&(c=app.lastLoaded=journal.archive.data[app.year].length-1);for(b[c].index=c;;)if(this.qualify(b[c],a)){this.html(journal.archive.data[app.year][c],-1==d?0:b[d].time.start||b[d].time.created);d=c;++app.displayedNum;app.displayedChars+=b[c].text.chars||0;app.displayedLines+=b[c].text.lines||0;if(b[c].time.end){var e=parseInt((b[c].time.end-
b[c].time.start)/6E4);isNaN(e)||(app.displayedTime+=e)}$("#search-result").hide().fadeIn(500);$("#total-displayed").text(app.displayedNum);$("#total-char").text(app.displayedChars);$("#total-line").text(app.displayedLines);$("#total-time").text(app.displayedTime);if($("#list").get(0).scrollHeight!=$("#list").height()||++c==journal.total)break}else if(this.htmlEmpty(),++c==journal.total)break;++c>=journal.total&&($(".loadmore").remove(),$("#list").append('<li><p class="separator"><span>EOF</span></p></li>'));
app.lastQualified=d;app.lastLoaded=c},qualify:function(a,b){if(!b)return!0;for(;-1!=b.search("  ");)b=b.replace("  "," ");for(var c=b.toLowerCase().split(" "),d=0,e=c.length;d<e;++d){var f=c[d].split("|"),g=!1;console.log("\t\t> Testing "+f);for(var h in f)if(f.hasOwnProperty(h)){var k;if("#"===f[h].charAt(0)){if(a.tags&&(k=!1,-1!==a.tags.split("|").indexOf(f[h].substr(1))&&(k=!0),k)){g=!0;break}}else if("%"===f[h].charAt(0)){k=!1;for(var l=app.tag().content(a.attachments),m=f[h].substr(1),n=0;n!==
l.length;++n)if(m==l[n]){k=!0;break}if(k){g=!0;break}}else if("@"===f[h].charAt(0)){if(k=f[h].substr(1),this.isInRange(k,a.time.created)){console.log("\t- Time match!");g=!0;break}}else if("+"===f[h].charAt(0)){if(a.text.body.match(new RegExp(f[h].substr(1),"i"))){g=!0;break}}else if(a.title.match(new RegExp(f[h],"i"))){g=!0;break}if(g)break}if(!g)return console.log("Reach end"),!1}return!0},date:function(a,b){var c=new Date(a),d=c.getHours(),e=c.getMinutes(),e=10>e?"0"+e:e,d=10>d?"0"+d:d;if(b)return d+
":"+e;var f=c.getFullYear(),g=new Date,h=g.getFullYear(),k=c.getMonth(),l=c.getDate();if(f===h)if(h=new Date(f,0,1),f=Math.floor((c-h)/864E5),g=Math.floor((g-h)/864E5),f===g)g="Today";else if(f+1===g)g="Yesterday";else switch(h=h.getDay(),f=Math.floor((f-h)/7),h=Math.floor((g-h)/7),g="",h-f){case 1:g="Last ";case 0:g+=app.weekArray[c.getDay()];break;default:g=app.monthArray[k]+" "+l}else g=app.monthArray[k]+" "+l;return g+" "+d+":"+e},html:function(a,b){a.summary=a.text.ext||a.text.body.substr(0,
...
720,80),e=app.util.style(e),f;if("images"==b&&(journal.archive.map[c.fileName]&&(f=journal.archive.map[c.fileName].url),void 0==f)||"video"==b&&(journal.archive.map[c.fileName+"_thumb.jpg"]&&(f=journal.archive.map[c.fileName+"_thumb.jpg"].url),void 0==f))return'<div class="dummy"></div>';e&&(e=' style="'+e+'"');e='<img src="'+f+'"'+e+">";Modernizr.canvas&&(e='<canvas width="160" height="160" data-src="'+f+'"></canvas>');1<c.urlType&&"weblink"==b&&(d='<span class="weblink-video"></span>');"video"==
b&&(d='<span class="video-play"></span>');d='<div class="thumb">'+e+""+d+"</div>"}return d||'<div class="dummy"></div>'},attached:function(a){var b=[];a=app.tag().content(a);for(var c=0,d=a.length;c<d;++c)b.push('<span class="'+a[c]+'"></span>');return b.join("")},isInRange:function(a,b){console.log("Call addLoadDataWithFilter.isInRange("+a+","+b+")");var c=a.split(":"),d=new Date(b);if(1==c.length){var e=c[0];if(4==e.length){var f=parseInt(e.substr(0,2)),g=parseInt(e.substr(2,2));return isNaN(f)||
isNaN(g)?!1:d.getYear()%100==g&&d.getMonth()+1==f}return 6==e.length?(f=parseInt(e.substr(0,2)),c=parseInt(e.substr(2,2)),g=parseInt(e.substr(4,2)),isNaN(f)||isNaN(c)||isNaN(g)?!1:d.getYear()%100==g&&d.getMonth()+1==f&&d.getDate()==c):!1}if(2==c.length){e=c[0];f=c[1];if(4==e.length){var c=parseInt(e.substr(0,2)),e=parseInt(e.substr(2,2)),h=parseInt(f.substr(0,2)),k;if(2==f.length)k=e;else if(4==f.length)k=parseInt(f.substr(2,2));else return!1;if(isNaN(c)||isNaN(e)||isNaN(h)||isNaN(k))return!1;f=d.getMonth()+
1;g=d.getYear()%100;return f>=c&&f<=h&&g>=e&&g<=k?!0:!1}if(6==e.length){c=parseInt(e.substr(0,2));d=parseInt(e.substr(2,2));e=parseInt(e.substr(4,2));h=c;k=e;if(2==f.length)g=parseInt(f);else if(4==f.length)h=parseInt(f.substr(0,2)),g=parseInt(f.substr(2,2));else if(6==f.length)h=parseInt(f.substr(0,2)),g=parseInt(f.substr(2,2)),k=parseInt(f.substr(4,2));else return!1;if(isNaN(c)||isNaN(d)||isNaN(e)||isNaN(h)||isNaN(g)||isNaN(f))return!1;h=new Date(2E3+k,h-1,g,23,59,59);return new Date(2E3+e,c-1,
...
window.calendar=function(){function a(a){for(var d in b)if(b.hasOwnProperty(d)&&a>d)return b[d];return""}var b={20:"bulb-7",15:"bulb-6",10:"bulb-5",7:"bulb-4",5:"bulb-3",3:"bulb-2",2:"bulb-1",1:"bulb-0"};return{viewTemplate:void 0,initView:function(){var a=new Date,b=[];a.setMonth(0,0);for(var e=[],f=0;f<(a.getDay()+1)%7;++f)e.push(0);a.setFullYear(a.getFullYear()+1);for(var g=1;12>=g;++g){a.setMonth(g,0);for(f=1;f<=a.getDate();++f)e.push(f);for(;42>e.length;)e.push(0);b.push(e);e=[];for(f=0;f<(a.getDay()+
1)%7;++f)e.push(0)}a=$(calendar.viewTemplate({days:b,monthName:app.monthArray}));$("#data-calendar").html(a)},showContent:function(b){b=journal.archive.data[app.year];for(var d=b.length,e=(new Date(app.year,0,1)).getTime(),f=!1,g=0,h=0;h!==d;++h){var k=b[h],l=k.time.start||k.time.created;if(864E5<=l-e||0>l-e)e=new Date(e),e=$(".calendar-table .month-"+e.getMonth()+" .day-"+e.getDate()),f&&e.addClass("article"),e.addClass(a(g)),f=!1,g=0,l=new Date(l),e=(new Date(app.year,l.getMonth(),l.getDate())).getTime();
k.contentType===app.contentType.BULB?++g:f=!0}},clear:function(){$(".calendar-table .day").removeClass("article bulb-0 bulb-1 bulb-2 bulb-3 bulb-4 bulb-5 bulb-6 bulb-7")}}}();
